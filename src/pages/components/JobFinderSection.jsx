...
JobFinderSection.propTypes = {
	sectionRef: PropTypes.shape({current: PropTypes.instanceOf(Element)}).isRequired
};

export default function JobFinderSection({sectionRef}) {
	...
	const images = [
		'/images/JobFinder/HomePage.png',
		'/images/JobFinder/Jobs.png',
		'/images/JobFinder/Contact.png',
		'/images/JobFinder/SignUp.png',
		'/images/JobFinder/LogIn.png',
		'/images/JobFinder/CreateJob.png',
		'/images/JobFinder/JobsProfile.png',
		'/images/JobFinder/JobContacts.png',
		'/images/JobFinder/EditJob.png',
		'/images/JobFinder/DeleteJob.png'
	];
		...
		<section ref={sectionRef} className="w-[200vw] min-h-screen text-white space-x-70">
			<div className="min-w-screen h-screen flex items-center space-x-70 ml-70">
				<article ref={text} className="w-100 h-auto flex flex-col space-y-10">
					<h1 className="text-6xl font-bold -ml-7">JobFinder</h1>
					<div className="self-center flex space-x-5 -ml-7">
						...
						<img src="/images/Icons/InertiaJS.svg" alt="InertiaJS" className="w-15 h-15" />
						<img src="/images/Icons/TailwindCSS.png" alt="TailwindCSS" className="w-18 h-18" />
						<img src="/images/Icons/Vite.png" alt="Vite" className="w-23 h-16" />
						<img src="/images/Icons/MySQL.png" alt="MySQL" className="w-23 h-16" />
					</div>
					<ul className="list-disc marker:text-white space-y-2">
						<li className="text-xl font-medium">Procurar trabalho</li>
						<li className="text-xl font-medium">Meter trabalho</li>
						<li className="text-xl font-medium">Contactar trabalho</li>
					</ul>
					<button onMouseEnter={() => tl.current.play()} onMouseLeave={() => tl.current.reverse()} className="w-45 h-15 rounded-full border-3 border-white text-white text-lg font-bold cursor-pointer"><a href="https://github.com/xXD4rkSoulXx/JobFinder">Ver Github</a></button>
				</article>
				<article ref={project} className="w-100 h-130 flex flex-col justify-center items-center space-y-10">
					<img ref={(el) => {currentImage.current = el; nextImage.current = el;}} key={images[index]} src={images[index]} alt="JobFinder" className="max-w-150 max-h-120" />
				</article>
			</div>
		</section>
	);
